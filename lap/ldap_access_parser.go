//downloaded from
//https://github.com/aidan-/ldap-access-parser
//license at
//https://github.com/rainsun/ldap-access-parser/blob/master/LICENSE
//syslog support added via example from
...
func init() {
  ...
  c.UseSyslog = flag.Bool( "syslog", false, "write output to stdout (Default: syslog)" )
...
func main() {
  ...
  if len(flag.Args()) < 1 {
    ...
    flag.Usage()
  } else if len(flag.Args()) > 1 && *c.TailFile {
    fmt.Println("Ignoring -Tail with multiple input files.")
    *c.TailFile = false
  }

  //prepare logging to syslog
  if *c.UseSyslog {
    sysLog, err := syslog.New( syslog.LOG_INFO, "ldap_access_parser" )
    if err != nil {
      log.Fatal( err )
    }
    c.Output = sysLog
  ...
  for _, fname := range flag.Args() {
    c.parseFile(activeConnections, fname)
  }
